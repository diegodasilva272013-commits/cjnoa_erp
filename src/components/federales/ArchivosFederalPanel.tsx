...
import {
  useFederalesDocs, uploadFederalDoc, deleteFederalDoc,
  downloadFederalDoc, getFederalDocSignedUrl, FederalDoc,
} from '../../hooks/useFederalesDocs';
...
function Thumb({ doc }: { doc: FederalDoc }) {
  const [url, setUrl] = useState<string | null>(null);
  const kind = kindOf(doc.mime, doc.nombre);
  useEffect(() => {
    let alive = true;
    if (kind === 'image') {
      getFederalDocSignedUrl(doc.storage_path).then(u => { if (alive) setUrl(u); });
    }
    return () => { alive = false; };
...
function Preview({ doc, onClose }: { doc: FederalDoc; onClose: () => void }) {
  const [url, setUrl] = useState<string | null>(null);
  const kind = kindOf(doc.mime, doc.nombre);
  useEffect(() => {
    let alive = true;
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/85 backdrop-blur-md p-4" onClick={onClose}>
      <div className="w-full max-w-5xl max-h-[92vh] flex flex-col bg-[#0a0a0a] border border-white/10 rounded-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => downloadFederalDoc(doc)}
        ...
        <div className="flex-1 overflow-auto bg-black flex items-center justify-center min-h-[40vh]">
          {!url ? (
            ...
            <div className="text-center p-8 text-gray-400 text-sm">
              ...
              <button onClick={() => downloadFederalDoc(doc)} className="mt-3 btn-primary text-xs">Descargar</button>
            </div>
          )}
        </div>
      </div>
...
export default function ArchivosFederalPanel({ clienteId }: { clienteId: string }) {
  const { user } = useAuth();
  const { showToast } = useToast();
  const { docs, loading } = useFederalesDocs(clienteId);
  const [uploading, setUploading] = useState(false);
  const [preview, setPreview] = useState<FederalDoc | null>(null);
  ...
  async function handleFiles(files: FileList | File[]) {
    ...
    for (const f of arr) {
      const r = await uploadFederalDoc(clienteId, f, user?.id || null);
      if (r.ok) okCount++;
  ...
  async function handleDelete(doc: FederalDoc) {
    const r = await deleteFederalDoc(doc);
              ...
              <div key={d.id} className="group rounded-xl bg-white/[0.03] border border-white/[0.06] overflow-hidden hover:border-violet-500/30 transition-colors">
                ...
                <div className="px-2 py-1.5 space-y-0.5">
                  ...
                  <div className="flex items-center justify-end gap-1 pt-1">
                    ...
                    <button onClick={() => downloadFederalDoc(d)} title="Descargar"
