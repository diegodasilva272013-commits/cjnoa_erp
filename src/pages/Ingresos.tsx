...
export default function Ingresos() {
  ...
  const periodo = usePeriodoFinanciero('ingresos');
  ...
  const filtrados = useMemo(() => {
    return ingresos.filter((i: IngresoOperativo) => {
      ...
      if (periodo.desde && i.fecha < periodo.desde) return false;
      if (periodo.hasta && i.fecha > periodo.hasta) return false;
      return true;
    });
  }, [ingresos, busqueda, filtroDoctor, filtroRama, filtroFuente, filtroModalidad, periodo.desde, periodo.hasta]);
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && filtrados.length === 0 && ingresos.length > 0 && (
        <EmptyPeriodHint periodo={periodo} recurso="ingresos" />
      )}
      ...
      <div className="bg-white/[0.02] border border-white/10 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-sm text-zinc-400">
            {ingresos.length === 0
              ? 'Aún no hay ingresos cargados.'
              : `Sin ingresos en ${periodo.label}.`}
          </div>
        ) : (
